...
def XYVxVy_init_circular():
    ...
    while(1):
        A_total_extra  *= (1+plus_factor)
        typeArea_extra *= (1+plus_factor)
        print(A_total_extra)
        R_WT_extra =  (typeArea_extra[WT_CELL_TYPE]/np.pi)**0.5
        R_total_extra = np.max(  [(A_total_extra/np.pi)**0.5  , R_WT_extra + 2*np.mean(cellR)] )
        
        
        if Init_numbers[0] == 0:
            R_WT_extra = 0
        elif Init_numbers[1] == 0:
            R_total_extra = R_WT_extra
        else:
            pass
            # alpha = np.linspace(0, np.pi, 1000)
            # A_0_candidate = (np.pi-alpha+np.sin(alpha)*np.cos(alpha)) * R_total_extra**2
        ...
        for cellC_1 in range(NCells):
            ...
            while (condCell == 0):
                ...
                if cellType[cellC_1]==WT_CELL_TYPE:
                    ...
                    random_theta = np.random.uniform(low=0, high=2*np.pi, size=1)
                    random_R   = np.random.uniform(low=0, high=1.0, size=1)*(R_total_extra-R_WT_extra)+R_WT_extra
                    cellX[cellC_1]= random_R * np.cos(random_theta)
                    cellY[cellC_1]= random_R * np.sin(random_theta)
                
                condSeparation = \
                ( (cellType[cellC_1]==0) and (cellX[cellC_1]**2+cellY[cellC_1]**2<R_WT_extra**2) )\
                or \
                ( (cellType[cellC_1]==1) and (cellX[cellC_1]**2+cellY[cellC_1]**2>R_WT_extra**2) )
...
for i in range(1):
    ...
    cellX, cellY, cellVx, cellVy = XYVxVy_init_circular()
