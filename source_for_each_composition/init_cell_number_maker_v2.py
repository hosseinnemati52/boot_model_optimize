...
if np.mean(WT_init_frac_interval) > (1.0 - eps):
    ...
    if np.max(Init_numbers_array) <= max_number:
        ...
        for j in range(np.shape(Init_numbers_array)[1]):
            if np.max(Init_numbers_array[:,j]) > max_number:
                while 1:
                    chosen_index = np.random.choice(all_indices, 1, replace=False)
                    if sample_bank[chosen_index] <= max_number:
                        Init_numbers_array[0,j] = sample_bank[chosen_index]
                        Init_numbers_array[1,j] = 0
                        break
        ...
        for j in range(np.shape(Init_numbers_array)[1]):
            if np.max(Init_numbers_array[:,j]) > max_number:
                while 1:
                    chosen_index = np.random.choice(all_indices, 1, replace=False)
                    if sample_bank[chosen_index] <= max_number:
                        Init_numbers_array[1,j] = sample_bank[chosen_index]
                        Init_numbers_array[0,j] = 0
                        break
    ...
    if np.max(Init_numbers_array) <= max_number:
        ...
        for j in range(np.shape(Init_numbers_array)[1]):
            if np.max(Init_numbers_array[:,j]) > max_number:
                while 1:
                    chosen_index = np.random.choice(all_indices, 1, replace=False)
                    if np.max(sample_bank[chosen_index,:]) <= max_number:
                        Init_numbers_array[0,j] = sample_bank[chosen_index,0]
                        Init_numbers_array[1,j] = sample_bank[chosen_index,1]
                        break
